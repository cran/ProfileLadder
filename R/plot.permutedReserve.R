...
#' in the package \code{ChainLadder}). Nonparametric (so-called functional-based)
#' methods include three algorithms implemented in the \code{ProfileLadder} 
...
plot.permutedReserve <- function(x, ...){
    ...
    graphics::boxplot(pReserve, main = paste("Permutation bootstrap for ", method, " (B = ", B, ")", sep = ""), 
                      cex = 0.8,  xlab = "Permutation bootstrap reserve",
                      horizontal = T, ylim = ylimits, pch = 22, col = "#D6EDF3")
    graphics::lines(rep(mean(pReserve), 2),  c(0.7, 1.3), col = "red", lwd = 4)
    graphics::text(mean(pReserve) + offSet, 0.72, "Bootstrap Average", col = "red", pos = 4, cex = 0.8)
    
    if (!is.na(reserve[4])){
      if (reserve[4] > reserve[3]){
        graphics::lines(rep(reserve[4], 2), c(0.6, 1.4), col = "#666666", lwd = 4)
        graphics::text(reserve[4] + offSet, 0.62,
                       "True reserve", col = "#666666" , pos = 4, cex = 1)
        
        graphics::lines(rep(reserve[3], 2), c(0.6, 1.4) , col = "darkred", lwd = 3)
        graphics::text(reserve[3] - offSet, 0.62,
                       paste(method, " reserve", sep =""), col = "darkred" , 
                       pos = 2, cex = 1)
      } else {
        graphics::lines(rep(reserve[4], 2), c(0.6, 1.4), col = "#666666", lwd = 4)
        graphics::text(reserve[4] - offSet, 0.62, 
                       "True reserve", col = "#666666" , pos = 2, cex = 1)
    ...
    graphics::hist(pReserve, xlab = "Permuted (bootstrap) reserves", ylab = "",  
                   main = paste("Permutation bootstrap (B = ",B,")", sep = ""), 
                   breaks = max(n, B/n), xlim = xLim, ylim = c(0, 1.5 * yLim), 
                   freq = F, cex = 0.8, col = "#D6EDF3")
    graphics::box()
    ...
    if (!is.na(reserve[4])){
      ...
      graphics::lines(rep(reserve[4], 2), c(0, 1.1 * yLim), 
                      col = "#666666", lwd = 3)
      graphics::legend("topleft", legend = c("Permutation bootstrap reserve (density)", 
                                              paste(method, "predicted reserve", sep = " "), 
                                              "True (unknown) reserve"), pch = 22, 
                                   pt.bg = c("red", "darkred", "#666666"), 
